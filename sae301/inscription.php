<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" type="text/css" href="style.css">
</head>

<body>
    <div class="form">
        <form method="POST" action="inscriTraitement.php" class="form">
            <input type="text" placeholder="Votre Prénom" id="prenom" name="prenom" class="champ">
            <input type="text" placeholder="Votre Nom" id="nom" name="nom" class="champ">
            <input type="text" placeholder="Votre Email" id="mail" name="mail" class="champ">
            <input type="password" placeholder="Votre Mot de passe" id="mdp" name="mdp" class="champ">
            <a href="connexion.php">J'ai déjà un compte</a>
            <input type="submit" id="submit" name="submit" class="champ">
        </form>
    </div>
</body>

</html>